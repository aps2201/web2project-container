...
To create a mysql database you can use the [official docker image](https://hub.docker.com/_/mysql/) and add it to the pod: `podman run --replace --name w2p-mysql --pod w2p -e MYSQL_ROOT_PASSWORD=[root_assword] -e MYSQL_DATABASE=w2p -e MYSQL_USER=w2p -e MYSQL_PASSWORD=[db_password] -d docker.io/library/mariadb:lts`
